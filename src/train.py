...
if __name__ == '__main__': 
    ...
    # 🚀 MODIFICAÇÃO GPU: MOVER MODELO PARA GPU
    ...
    criterion = criterion.cuda()  # 🚀 MODIFICAÇÃO: CRITÉRIO NA GPU
        ...
        for epoch in range(epochs): 
            with training_handler.create_training_progress() as epoch_progress:
                ...
                for batch_idx, batch in enumerate(train_dataloader): 
                    ...
                    # 🚀 MODIFICAÇÃO GPU: MOVER DADOS PARA GPU
                    X = X.cuda()  # ← ADICIONADO: Input data para GPU
                    
                    # Converter labels para GPU
                    y_gpu = []
                    for t in y:
                        y_gpu.append({
                            'labels': t['labels'].cuda(),    # ← ADICIONADO
                            'boxes': t['boxes'].cuda()       # ← ADICIONADO
                        })
                    y = y_gpu  # ← ADICIONADO: Labels na GPU
                ...
                with torch.no_grad():
                    for batch_idx, batch in enumerate(test_dataloader):
                        ...
                        # 🚀 MODIFICAÇÃO GPU: MOVER DADOS DE TESTE PARA GPU
                        X = X.cuda()  # ← ADICIONADO
                        
                        # Converter labels de teste para GPU
                        y_gpu = []
                        for t in y:
                            y_gpu.append({
                                'labels': t['labels'].cuda(),    # ← ADICIONADO
